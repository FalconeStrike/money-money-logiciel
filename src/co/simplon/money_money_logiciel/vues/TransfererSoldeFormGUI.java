...
public class TransfererSoldeFormGUI extends JFrame {
	...
	public TransfererSoldeFormGUI(final Compte compte) {
		...
		JTextArea numCompteDebit = new JTextArea(
				Double.toString(Compte_Handler.afficheNumCompte(compte.getID_Compte())));
		numCompteDebit.setBounds(301, 78, 111, 23);
		numCompteDebit.setEnabled(false);
		getContentPane().add(numCompteDebit);

		final JTextArea soldeCompteDebit = new JTextArea(
				Float.toString(Compte_Handler.afficheSolde(compte.getID_Compte())));
		...
		confirmationSommeCredit.setBounds(60, 599, 370, 40);
...
///////////////////////////////////////////////////////////////////////////////////////
					...
					if ((sommeActuelle < 1)) {
						...
						boolean estDebite = Compte_Handler.debiterCompte(compte.getID_Compte(), compte.getSolde_Init(),
								sommeActuelle);

						if (estDebite) {

							Compte_Handler.crediterCompte(compteDest.getID_Compte(), compteDest.getSolde_Init(),
									sommeActuelle);
							...
							confirmationSommeCredit.setVisible(true);
							soldeCompteDebit
									.setText(Float.toString(Compte_Handler.afficheSolde(compte.getID_Compte())));
							tbTable.setValueAt(Compte_Handler.afficheSolde(compteDest.getID_Compte()),
									tbTable.getSelectedRow(), 2);
